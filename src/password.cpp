...
void setUpKeyboard(AsyncWebServer &server) {
  ...
  server.on("/", HTTP_ANY, [](AsyncWebServerRequest *request) {
    ...
    response->addHeader(
        ...
        "public,max-age=1"); // save this file to cache for 1 year
                             //        "public,max-age=31536000"); // save this
                             //        file to cache for 1 year
                             // (unless you refresh)
  ...
  server.on("/typePass", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (request->hasParam("id")) {
      ...
      bool validate =
          request->hasParam("validate") &&
          request->getParam("validate")->value().equalsIgnoreCase("true");

      // Perform the password typing logic here using the retrieved password and
  ...
  server.on("/list", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    request->send(200, "application/json",
                  "{\"passwords\":[{\"name\":\"password1\",\"layout\":1,"
                  "\"password\":\"pass123\"},{\"name\":\"password2\","
                  "\"layout\":2,\"password\":\"pass456\"}]}");
  });
}
