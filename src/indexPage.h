...
async function togglePasswordVisibility(visible){passwordInput.type=visible?"text":"password";if(ui_data.mode=="edit"&&passwordInput.value.length==0){const pass=await fetch(`/fetchPass?id=${positionSelect.value}`);passwordInput.value=JSON.parse(await pass.text());}}
...
document.addEventListener("DOMContentLoaded",function(){layoutSelect.addEventListener("change",function(){layoutIndex.value=layoutSelect.selectedIndex;});layoutIndex.value=layoutSelect.selectedIndex;editFormContent.addEventListener("submit",editFormHandler);initToggleButtons();});window.onload=async()=>{await getPasswords();};</script>
</body>
</html>
<!-- vim:ts=2:sw=2:et:
-->
