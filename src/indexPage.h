...
return password;}
function generatePass(){const base_length=UserPreferences.get("password_length");const length=prompt(`Password length (default ${base_length}):`,base_length,);if(!length||isNaN(length)||length<8){return;}
...
function toggleButton(buttonElement,options={}){const setting=buttonElement.getAttribute("data-setting")||options.setting;const currentState=UserPreferences.get(setting)!==false;const newState=options.noflip?currentState:!currentState;UserPreferences.save(setting,newState);const mode=newState?"enabled":"disabled";buttonElement.textContent=buttonElement.getAttribute(`data-${mode}-text`);const fn=buttonElement.getAttribute("data-changed")||(()=>{});wiggle(buttonElement);eval(fn)(newState);}
