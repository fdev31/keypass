...
document.addEventListener("DOMContentLoaded",function(){layoutSelect.addEventListener("change",function(){layoutIndex.value=layoutSelect.selectedIndex;});layoutIndex.value=layoutSelect.selectedIndex;editFormContent.addEventListener("submit",editFormHandler);initToggleButtons();});window.onload=async()=>{await checkPassphrase();await getPasswords();};</script>
</body>
</html>
<!-- vim:ts=2:sw=2:et:
-->
