...
<textarea id="blobTextInput" placeholder="Paste blob file content here..." rows="4" style="width: 100%;"></textarea>
</div>
...
function uploadBlobFile(){passwdRecoveryInputs.style.display='block';const fileInput=document.getElementById('blobFileInput');const textInput=document.getElementById('blobTextInput');let contentToUpload;let contentSource;if(fileInput.files&&fileInput.files.length>0){contentToUpload=fileInput.files[0];contentSource='file';}
else if(textInput.value.trim()){const pastedText=textInput.value.trim();contentToUpload=new Blob([pastedText],{type:"text/plain"});contentSource='text';}
...
return response.text();}).then((result)=>{fileInput.value='';textInput.value='';button.textContent=originalText;button.disabled=false;alert(result);setTimeout(()=>{window.location.reload();},1000);}).catch((error)=>{button.textContent=originalText;button.disabled=false;alert(`Error: ${error.message}`);});}
function initializeRecoveryInputs(){const container=document.getElementById('passwdRecoveryInputs');const fileInput=document.getElementById('blobFileInput');const textInput=document.getElementById('blobTextInput');container.style.display='none';container.style.marginTop='10px';container.style.marginBottom='10px';fileInput.style.display='none';const buttonRow=document.createElement('div');buttonRow.className='button-row';buttonRow.style.marginBottom='10px';const fileButton=document.createElement('button');fileButton.className='modern-btn';fileButton.textContent='Select File';fileButton.onclick=function(e){e.preventDefault();fileInput.click();};const textButton=document.createElement('button');textButton.className='modern-btn';textButton.textContent='Use Text Input';textButton.onclick=function(e){e.preventDefault();textInput.focus();};buttonRow.appendChild(fileButton);buttonRow.appendChild(textButton);const fileNameDisplay=document.createElement('div');fileNameDisplay.style.fontSize='14px';fileNameDisplay.style.color='rgba(255, 255, 255, 0.8)';fileNameDisplay.style.marginBottom='10px';fileNameDisplay.style.display='none';fileInput.addEventListener('change',function(){if(fileInput.files.length>0){fileNameDisplay.textContent=`Selected file: ${fileInput.files[0].name}`;fileNameDisplay.style.display='block';textInput.value='';}else{fileNameDisplay.style.display='none';}});container.insertBefore(fileNameDisplay,container.firstChild);container.insertBefore(buttonRow,container.firstChild);}
